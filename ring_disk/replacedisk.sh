...
DISKDUMP=no	# it will do a bizols before and after the operation and compare 
...
if [ $? == 0 ]; then
  echo "device ${SCAL_MP}/${TARGET} is busy"
  exit
fi
echo "Rebuilding device $DEVICE mounted on ${TARGET_MP} disk ${TARGET} UUID $UUID"
umount ${TARGET_MP}
findmnt ${TARGET_MP} 
A=$? 
if [ $A -eq 0 ] ; then 
